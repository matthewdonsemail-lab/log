...
| **Public read API** | The dashboard's API page makes real keys (`lk_api_` plus 64 hex characters, at most 5 per person, only the SHA-256 stored, shown once, revocable) that read your own data over HTTP: `GET /api/v1/me` (plan and usage), `/keywords` (phrases with counts and freshness) and `/matches` (newest first, with the post and score; `limit`, `platform`, `min_score` and a `before` cursor). Read-only: no write route exists. 60 requests a minute per key (`X-RateLimit-*` headers, 429 with `Retry-After`), one JSON error shape, no CORS. An ingest key does not work here and an API key does not work on the ingest side. | [`apiKeys.ts`](convex/apiKeys.ts), [`publicApi.ts`](convex/publicApi.ts), [`http.ts`](convex/http.ts), [`DashboardApiLive.tsx`](apps/web/src/components/DashboardApiLive.tsx) |
