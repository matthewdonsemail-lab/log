...
| **Mandatory proxy** | Every X and Facebook helper must browse through a proxy the **operator** sets on the deployment (`PROXY_URL`, like `http://user:pass@host:port`, or `socks5://`). People never choose, type or see one: the proxy field is gone from Settings and the extension. A helper fetches it from `GET /proxy` with its ingest key (401 without one), passes it to Chrome, and refuses to start if the deployment has none (503, "Reading is paused until the operator sets up the proxy"). There is no flag to skip it. The proxy is never logged or shown, its parts are added to the helper's hidden-secrets list, and no other endpoint returns it. `PROXY_REQUIRED=false` is an operator-only switch for a deployment that has no proxy yet. Honest limit: the helper runs on the person's own computer, so a technical person could still read the proxy from their own machine; only server-side reading (not built) would fully prevent that. Reddit is read by the Convex server, which cannot use a proxy. | [`lib/proxy.ts`](convex/lib/proxy.ts), [`http.ts`](convex/http.ts), [`clients/listeningkit_ingest.py`](clients/listeningkit_ingest.py) |
