...
Real on Convex: sign-in, feed, keywords, matches, scoring, the Reddit cron, ingest keys, connected accounts, and the ingest, session and phrases endpoints. X works through the helper above (run against a real throwaway account: 20 real tweets became matches); Facebook has its helper too, checked against stand-in pages and the real site's login wall but not yet run with a real Facebook login. Still the in-browser mock described above: messaging, listings, groups, brand, analytics, API keys, and the onboarding brand step. Phone alerts (Bark) and Firecrawl discovery are on the to-do list in [`HANDOFF.md`](HANDOFF.md).
