...
The dashboard talks to **Convex directly**. There is no bridge server on the live path. Reddit works end to end; X and Facebook are read by helpers on the person's own computer.
...
| **Website reading (Firecrawl)** | In onboarding the person pastes their website; the `brand:extractFromWebsite` action sends it to Firecrawl's scrape API asking for structured JSON (name, tagline, offerings, tone, location, logo), cleans every field (website text is untrusted; filler like "N/A" is dropped), stores it in the `brands` table and lays it over the brand. A short summary of the business also rides on every scoring call, so scores reflect fit to the person's own business. Needs sign-in, allows one read every 30 seconds (each read spends Firecrawl credits) and refuses non-public addresses. Without `FIRECRAWL_API_KEY` onboarding keeps the name guessed from the domain. | [`brand.ts`](convex/brand.ts), [`lib/firecrawl.ts`](convex/lib/firecrawl.ts) |
| **Email alerts (AgentMail)** | Settings, Notifications: a person saves one email address, picks a score bar and switches alerts on. A 5-minute cron (`alerts:sweep`) sends each person one plain-text email with their strongest new matches through AgentMail's send API: at most 5 matches per email, at most one email per person every 10 minutes, only matches scored in the last day, each match at most once. A failed send is never marked as sent and is retried. The subject is fixed text, the body is text only (never HTML), and the address must be a single plain address. "Send a test email" is limited to one a minute. Needs `AGENTMAIL_API_KEY` and `AGENTMAIL_INBOX_ID`. | [`alerts.ts`](convex/alerts.ts), [`lib/agentmail.ts`](convex/lib/agentmail.ts) |
...
| **Mandatory proxy** | Every X and Facebook helper must browse through a proxy the **operator** sets on the deployment (`PROXY_URL`, like `http://user:pass@host:port`, or `socks5://`). People never choose, type or see one: the proxy field is gone from Settings and the extension. A helper fetches it from `GET /proxy` with its ingest key (401 without one), passes it to Chrome, and refuses to start if the deployment has none (503, "Reading is paused until the operator sets up the proxy"). There is no flag to skip it. The browser skips pictures, video and fonts (the helper only reads text), because a proxy is paid for by the gigabyte. The proxy is never logged or shown, its parts are added to the helper's hidden-secrets list, and no other endpoint returns it. `PROXY_REQUIRED=false` is an operator-only switch for a deployment that has no proxy yet. Honest limit: the helper runs on the person's own computer, so a technical person could still read the proxy from their own machine; only server-side reading (not built) would fully prevent that. Reddit is read by the Convex server, which cannot use a proxy. | [`lib/proxy.ts`](convex/lib/proxy.ts), [`http.ts`](convex/http.ts), [`clients/listeningkit_ingest.py`](clients/listeningkit_ingest.py) |
| **Plans (Free enforced, Pro announced)** | Everyone is on the Free plan, enforced on the server: **1 phrase per platform and 1 connected account per platform at a time**. `keywords:create` and the account creators refuse the next one with a plain message; phrases or accounts someone already had are never removed, only new ones are blocked. A pricing section on the landing page and Settings, Billing (plan and usage, from `plan:mine`) show it, and the Keywords page disables the button with a notice when a platform is full. Pro ($19 a month, planned) is shown as coming soon: nothing charges, there is no card form, and there is no way to buy. The operator can raise the limits for a whole deployment with `PLAN_PHRASES_PER_PLATFORM` and `PLAN_ACCOUNTS_PER_PLATFORM`; people cannot. | [`lib/plan.ts`](convex/lib/plan.ts), [`plan.ts`](convex/plan.ts), [`landing/Pricing.tsx`](apps/web/src/landing/Pricing.tsx) |
| **Public API (scoped keys, phrase writes, webhooks)** | The dashboard's API page makes real keys (`lk_api_` plus 64 hex characters, at most 5 per person, only the SHA-256 stored, shown once, revocable) with **scopes** chosen when the key is made: `read` (always), `write:phrases`, `webhooks`. **Read:** `GET /api/v1/me`, `/keywords`, `/keywords/{id}`, `/matches` (`limit`, `platform`, `min_score`, a `before` cursor) and `/matches/{id}`. **Write phrases:** `POST /keywords` (with an `Idempotency-Key`), `PATCH` and `DELETE /keywords/{id}`, through the same code and plan limits as the dashboard. **Webhooks (a Pro feature; none on Free, and the operator can switch them on):** `POST/GET /webhooks`, `PATCH/DELETE /webhooks/{id}`, `POST /webhooks/{id}/test`, `GET /webhooks/{id}/deliveries`. When a match is scored at or above a webhook's level, ListeningKit POSTs a signed event (HMAC-SHA256 over `timestamp.body`), retries at 1, 5 and 30 minutes, and switches the webhook off after 5 failed deliveries in a row. Addresses must be public https names (no IPs, no private or reserved names, no ports, no credentials), are checked again at send time, redirects are never followed and responses are never read. 60 requests a minute per key (`X-RateLimit-*` headers, 429 with `Retry-After`), one JSON error shape, no CORS. An ingest key does not work here and an API key does not work on the ingest side. | [`apiKeys.ts`](convex/apiKeys.ts), [`publicApi.ts`](convex/publicApi.ts), [`webhooks.ts`](convex/webhooks.ts), [`http.ts`](convex/http.ts), [`DashboardApiLive.tsx`](apps/web/src/components/DashboardApiLive.tsx) |
| **MCP server (for AI agents)** | `POST /mcp` on the site address: a Model Context Protocol server (JSON-RPC over HTTP, one message per request, no sessions) that lets Claude, Cursor, Hermes or any MCP client use ListeningKit as tools. It uses the same `lk_api_` keys, scopes, per-minute allowance and plan limits as the REST API. Tools: `get_plan`, `list_keywords`, `list_matches`, `get_match` (scope `read`) and `add_keyword`, `set_keyword_status`, `remove_keyword` (scope `write:phrases`, the last marked destructive). A key is only shown the tools its scopes allow. Post text is labelled as untrusted data. ChatGPT connectors cannot send an API key, so ChatGPT is documented as not supported yet. | [`lib/mcp.ts`](convex/lib/mcp.ts), [`http.ts`](convex/http.ts), [`mcp.mdx`](apps/docs/content/docs/guide/mcp.mdx) |
...
| **X helper** | `clients/x_push.py` runs on the person's own computer: loads their connected X login (`GET /session`), asks for their X phrases (`GET /phrases`), searches X's Latest tab in a real Google Chrome window (driven by Playwright, logged in with that login; needs Chrome installed) for each exact phrase, and pushes tweets to `/ingest`. It reads the page like a person, so it does not depend on X's private API (the twikit engine broke when X rebuilt its site: "Couldn't get KEY_BYTE indices"; it is kept as `--engine twikit`). It spaces requests, stops when X asks it to slow down, and reports a refused login in plain words. Automating a logged-in X session can go against X's terms; use an account you can afford to lose. | [`clients/x_push.py`](clients/x_push.py), [`clients/x_browser.py`](clients/x_browser.py), [`http.ts`](convex/http.ts) |
| **Facebook helper** | `clients/facebook_push.py` is the X helper's twin: it loads the connected Facebook login, asks for the person's Facebook phrases, searches Facebook's recent posts in real Chrome for each exact phrase and pushes them to `/ingest`. Facebook has no public search API, so it reads the page like a person; it only sees what that account may see (public posts and groups it has joined). Facebook fills in a post's address only while the mouse is over its timestamp, so the reader hovers each one to read it (a post whose address cannot be read gets a stable id instead). It keeps only posts that contain the phrase, drops ads, and tells a refused login (Facebook answers search with a bare "Not Found"), a security checkpoint and a slow-down apart. Automating a logged-in Facebook session can go against Facebook's terms; use an account you can afford to lose. | [`clients/facebook_push.py`](clients/facebook_push.py), [`clients/facebook_browser.py`](clients/facebook_browser.py) |
...
| `FIRECRAWL_API_KEY` | Convex deployment env | Lets onboarding read the person's website (Firecrawl scrape API). Optional; without it the brand name is guessed from the domain. |
| `AGENTMAIL_API_KEY`, `AGENTMAIL_INBOX_ID` | Convex deployment env | Send email alerts through AgentMail. `AGENTMAIL_INBOX_ID` is the inbox the alerts come from (create it in the AgentMail console). Optional; without them the Settings form says alerts are not switched on. |
| `PROXY_URL` | Convex deployment env | The proxy every X and Facebook helper must use (see *Mandatory proxy*). Helpers refuse to run on a deployment without it. |
| `PROXY_REQUIRED` | Convex deployment env | Set to `false` (operator only) to let helpers browse directly on a deployment that has no proxy yet. Anything else, or unset, keeps the proxy mandatory. |
| `PLAN_PHRASES_PER_PLATFORM`, `PLAN_ACCOUNTS_PER_PLATFORM` | Convex deployment env | Operator-only: raise the Free plan's limits (default 1 each, a whole number from 1 to 1000) for a deployment, for example while recording a demo. Unset keeps the Free limits. |
| `PLAN_WEBHOOKS_PER_PERSON` | Convex deployment env | Operator-only: how many webhooks each person may have. Default 0 (webhooks are a Pro feature, and Pro is not available yet). Set it to a number from 1 to 1000 to switch webhooks on for the deployment. |
...
### Multi-account connections and the mandatory proxy

Connect **every account you own**, each with its own cookie. Accounts are **id-keyed** (not platform-keyed), so multiple accounts per platform are first-class. The store and cookie validation live in [`lib/connections/`](apps/web/src/lib/connections/store.ts); the extension handshake side is documented at [docs /getting-started/chrome-extension](apps/docs/content/docs/getting-started/chrome-extension.mdx).

**Proxies are not the person's to choose.** There is no proxy field anywhere in the app or the extension. X and Facebook reading always goes through a proxy the operator sets once on the deployment (`PROXY_URL`); see the *Mandatory proxy* row in the security table.
