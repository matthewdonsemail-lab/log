...
/** What we ask Firecrawl's model to pull out of the page. Everything is optional: a thin page yields a thin answer, not an invented one. */
...
export function normalizeWebsite(input: string): { ok: true; url: string } | { ok: false; reason: string } {
  ...
  let url: URL
  try { url = new URL(withScheme) } catch { return { ok: false, reason: 'That address does not parse. Check it and try again.' } }
...
/** What an extraction model writes when the page did not say: it is an absence, not a fact, so it is never stored. */
...
/** Keeps only a logo that is an absolute https address, so a page cannot inject anything else into the app. */
...
/**
 * The website's words are untrusted: every field is checked, trimmed and capped, and anything unexpected is
 * dropped. Null when there is no usable business name, so nothing is invented from an empty page.
 */
export function parseBrandFacts(raw: unknown): BrandFacts | null {
  ...
  return {
    name, tagline: clean(row.tagline, 200) ?? '', offerings,
    ...(tone ? { tone } : {}), ...(formality ? { formality } : {}),
    ...(locationLabel ? { locationLabel } : {}), ...(logoUrl ? { logoUrl } : {}),
  }
}

/** One Firecrawl call. Errors say what happened in plain words and never include the response body or the key. */
...
/** A short summary of the business for the scoring prompt, or null when there is none. Kept small: it rides on every scoring call. */
