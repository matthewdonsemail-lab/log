...
| Dashboard pages | Feed, Keywords, Accounts, Docs, API, Settings, Messages (X) are real. **Groups, Listings, Brand and Analytics are still in-browser mock and are hidden from the live site's menu** (they redirect to the feed); they still show in demo mode. Analytics was wrongly reported real earlier: it uses `lib/analytics.ts` and `lib/keywords.ts` (the mock libraries), not Convex, and only surfaced once the account had a real phrase to compare against fabricated rows ("galway_mam", etc). Found and hidden 2026-09-22. |
| X direct messages | Real backend (`convex/messages.ts`, `/dm/*` routes) and helper (`clients/x_dm.py`), tested (8 backend, 9 Python, 4 web). **Live read is blocked today: X has moved this account's DMs to an encrypted "X Chat" that requires a device passcode before showing anything, with a different DOM than the classic DM UI the helper's selectors target.** Confirmed live on 2026-09-22: signed in, connected X, a real DM sent to the account showed `Welcome to the new X Chat / Set Passcode` instead of any inbox. Sending has not been tried live at all. Not a code bug in the backend; the browser selectors in `clients/x_browser.py` (`DM_CONVERSATIONS_JS`, `DM_MESSAGES_JS`, `send_dm`) need rewriting against X Chat's markup once someone sets a passcode on a test account to see it. |
