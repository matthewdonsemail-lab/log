...
Checks that were green at the last commit: backend 216, web 203, `clients` 86, reddit-camofox-client 25;
...
The dashboard's **API** page now makes real keys and there is a real read-only API behind them: `GET /api/v1/me`, `/keywords`, `/matches` (see `apps/docs/content/docs/guide/api.mdx`). Checked end to end on dev in a real browser: create a key, call all three endpoints, page through matches, then revoke and see 401. It is deployed to prod and checked there the same way (create a key in the live dashboard, call all three endpoints, revoke, see 401). The docs' old mock API Reference is now labelled "Legacy mock API" with a warning on every page, and pages that describe the earlier design carry a banner. The older mock API-key screens (scopes, activity console, key detail page) are still in the code and only show when the app is not on the live backend; remove them when convenient. Not built: write endpoints, scopes, webhooks, an official SDK.

## API v2: scopes, phrase writes, webhooks (2026-09-21)

Keys now carry scopes (`read`, `write:phrases`, `webhooks`), chosen with tick-boxes on the API page. Phrase writes (`POST/PATCH/DELETE /api/v1/keywords`) and webhooks (`/api/v1/webhooks...`) are built and were checked end to end on dev in a real browser and over real HTTPS: a key made in the dashboard, the Free plan limit refusing a second phrase, webhooks refused on Free, addresses like `127.0.0.1` and `nip.io` refused, a signed test event and a real scored match delivered to an outside receiver and verified there with an independent HMAC, nothing sent while switched off, then cleanup. **Webhooks are a Pro feature: `PLAN_WEBHOOKS_PER_PERSON` is 0, so nobody can use them until the operator raises it** (`pnpm exec convex env set --prod PLAN_WEBHOOKS_PER_PERSON 1`, and remove it to switch them off again). Not built: emailing the owner when a webhook is switched off, more event types than `match.created`, a signing-secret rotation endpoint, an SDK. The check on webhook addresses cannot resolve DNS (documented in the docs and AGENTS.md). Dev data touched by the test: two real X matches on the dev deployment now carry a score from the test.

## Prod check of everything built (2026-09-21)

Deployed the backend and site to prod and ran one full check on the live site (signed in, real services, cleaned up after itself): landing pricing and the website box; docs pages, the legacy-mock warning and the redirect for addresses without .html; Free plan billing; Firecrawl reading a real website in onboarding; the docs tab; the AgentMail test email (arrives); API keys with scopes; reads; phrase writes with the Free limit and idempotent retries; a read-only key refused; webhooks refused on Free and, with them switched on for the test only, address checks, secret shown once, a signed test event and a real scored match delivered to an outside receiver and verified there, the delivery log, switch-off and delete; the proxy endpoint (valid ingest key only) and the ingest door. All passed. Temporary things (the webhooks switch, keys, the test phrase) were removed; one orphan test post from the ingest step stays, with no match attached. Not re-run on prod: the X and Facebook helpers (they need an X or Facebook login connected to a prod account; they were checked through the proxy on dev) and OpenAI scoring (no key yet). Also seen working: the 5-minute alert run emailed a real "2 strong matches" digest after two dev matches were scored.

## MCP server (2026-09-21)

`POST /mcp` speaks the Model Context Protocol (JSON-RPC 2.0 over HTTP, no sessions, no stream, `GET` answers 405). It authenticates with the same `lk_api_` keys, counts every message against the same 60 a minute, and exposes seven tools (`get_plan`, `list_keywords`, `list_matches`, `get_match`, `add_keyword`, `set_keyword_status`, `remove_keyword`) filtered by the key's scopes. Setup for Claude Code, Claude Desktop, Cursor, Hermes and generic clients is in the docs page `guide/mcp`; the landing "Works with" icons and the dashboard API page point to it. **ChatGPT is documented as not supported**: its custom connectors use OAuth or no sign-in and cannot send an API key. Adding OAuth would be the next step. The Claude Desktop (`mcp-remote`), Cursor and Hermes snippets are written from their documented formats and were **not** run against those apps; the server itself is covered by 26 tests and answers 401 correctly on dev. Not built: resources, prompts, webhook tools, streaming.
