...
**Website reading** uses Firecrawl: in onboarding, paste your website and ListeningKit reads your business name and offerings from it (needs `FIRECRAWL_API_KEY`). **Email alerts** use AgentMail: strong matches are emailed to you (needs `AGENTMAIL_API_KEY` and `AGENTMAIL_INBOX_ID`). Both are optional and simply stay off when their keys are not set.

Still mock (in-browser demo data, not real): messaging, community listings and joins, the brand reveal after your website is read, analytics and API keys. See [HANDOFF.md](https://github.com/matthewdonsemail-lab/log/blob/main/HANDOFF.md) for the full to-do list.
...
<Card title="The API" href="/docs/guide/api">
  Read and manage your phrases and matches from your own code, with scoped keys and (on Pro) webhooks.
</Card>
