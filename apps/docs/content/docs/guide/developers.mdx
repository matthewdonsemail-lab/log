...
| `PLAN_PHRASES_PER_PLATFORM`, `PLAN_ACCOUNTS_PER_PLATFORM` | Operator-only: raise the Free plan limits (default 1 each) for the whole deployment. |
...
Read-only, at `/api/v1` on the deployment's site address: `GET /me`, `/keywords`, `/matches`, authenticated with a key made in the dashboard (`lk_api_...`, stored as a SHA-256 hash, shown once; `convex/apiKeys.ts`). Routes are in `convex/http.ts`, data in `convex/publicApi.ts`. 60 requests a minute per key (a counter on the key row), one JSON error shape, no CORS (keys must not live in browsers). Reference: [The API](/docs/guide/api).
...
| Task | Command |
|---|---|
| Backend tests (146) | `pnpm test:backend` |
| Web tests (194) | `pnpm --filter web test` |
