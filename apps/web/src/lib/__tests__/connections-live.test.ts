...
let account = { id: 'acc1', platform: 'reddit', label: 'Reddit', connectedAt: null as string | null }
...
beforeEach(() => {
  calls.length = 0
  sessions = []
  account = { id: 'acc1', platform: 'reddit', label: 'Reddit', connectedAt: null }
...
describe('Settings connections on the live backend', () => {
  it('connects with a real token and returns the connected account', async () => {
    const record = await connectAccount({ id: 'acc1', platform: 'reddit', cookie: redditToken })
  ...
  it('catches a token pasted on the wrong account before sending it', async () => {
    await expect(connectAccount({ id: 'acc1', platform: 'x', cookie: redditToken })).rejects.toThrow('That is a Reddit token')
    expect(calls).not.toContain('sessions:save')
  })

  it('tests a connection against what is saved', async () => {
    await expect(testConnection({ id: 'acc1', platform: 'reddit', cookie: '' })).rejects.toThrow('Not connected yet')
    sessions = [{ platform: 'reddit', expiresAt: Date.now() - 1000 }]
    await expect(testConnection({ id: 'acc1', platform: 'reddit', cookie: '' })).rejects.toThrow('expired')
    sessions = [{ platform: 'reddit', expiresAt: Date.now() + 86_400_000 }]
    expect(await testConnection({ id: 'acc1', platform: 'reddit', cookie: '' })).toEqual({ ok: true })
  })

  it('has no proxy for a person to set: a connection is just the token', async () => {
    const record = await connectAccount({ id: 'acc1', platform: 'reddit', cookie: redditToken })
    expect(record).not.toHaveProperty('viaProxy')
    sessions = [{ platform: 'reddit', expiresAt: null }]
    expect(await testConnection({ id: 'acc1', platform: 'reddit', cookie: '' })).not.toHaveProperty('viaProxy')
  })
