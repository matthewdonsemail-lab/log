...
describe('the dashboard menu', () => {
  ...
  it('hides the pages that only have mock data on the live site, and keeps the real ones', () => {
    ...
    expect(labels).toEqual(['Feed', 'Keywords', 'Analytics', 'Accounts', 'Docs', 'API', 'Settings'])
    expect(DEMO_ONLY_PATHS).toEqual(['/dashboard/groups', '/dashboard/facebook/listings', '/dashboard/brand', '/dashboard/messages'])
  })
})
