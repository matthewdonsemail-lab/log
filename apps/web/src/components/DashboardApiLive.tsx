...
function ScopeChip({ scope }: { scope: string }) {
  const writes = scope !== 'read'
  return (
    <span className={`rounded-md px-1.5 py-0.5 text-[11px] font-semibold ${writes ? 'bg-amber-100 text-amber-800' : 'bg-black/[0.04] text-text-secondary'}`}>
      {scope}
    </span>
  )
}

/** The API: keys you make here read, and if you allow it change, your own data over HTTP. What a key may do is chosen when it is made. */
export function DashboardApiLive() {
  ...
  async function create() {
    ...
    setCreating(true)
    try {
      setFresh(await createApiKey(label, scopes))
      setLabel('')
      setScopes(['read'])
      reload()
    } catch (err: unknown) {
  ...
  async function revoke(key: ApiKeyRow) {
    try {
      ...
      notifySuccess('Key revoked', `${key.label} can no longer use your data.`)
      reload()
    } catch (err: unknown) {
    ...
    <div className="flex flex-col gap-4 pb-6">
      <div>
        ...
        <p className="text-sm text-text-secondary">
          Use your phrases and matches from your own code. Each key belongs to you, sees only your data, and can do only what you allow when you make it.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-5">
        ...
        <fieldset className="flex flex-col gap-2">
          <legend className="mb-1 text-sm font-semibold text-slate-700">What this key can do</legend>
          {SCOPE_CHOICES.map((choice) => (
            <label key={choice.id} className="flex items-start gap-2 text-sm text-slate-700">
              <input
                id={`api-scope-${choice.id.replace(':', '-')}`}
                type="checkbox"
                className="mt-0.5 size-4"
                checked={scopes.includes(choice.id)}
                disabled={choice.locked}
                onChange={() => toggleScope(choice.id)}
              />
              <span><span className="font-semibold">{choice.label}</span> <span className="text-text-secondary">· {choice.help}</span></span>
            </label>
          ))}
        </fieldset>
        {scopesCanWrite(scopes) ? (
          <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">
            This key will be able to change your data. Keep it secret, and revoke it if it ever leaks.
          </p>
        ) : null}
      </div>
      ...
      {fresh ? (
        <div className="flex flex-col gap-3 rounded-xl border-2 border-[#2a8cff] bg-white p-5">
          ...
          <div className="flex flex-wrap gap-1.5">{fresh.scopes.map((scope) => <ScopeChip key={scope} scope={scope} />)}</div>
      ...
      <div className="flex flex-col gap-2">
        {(keys ?? []).map((key) => (
          <div key={key.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-4">
            <div className="flex min-w-0 flex-1 flex-col gap-1">
              ...
              <span className="flex flex-wrap gap-1.5">{key.scopes.map((scope) => <ScopeChip key={scope} scope={scope} />)}</span>
            </div>
        ...
        {keys !== null && keys.length === 0 ? (
          <p className="rounded-xl border border-dashed border-slate-300 p-5 text-sm text-text-secondary">
            No keys yet. Create one to start using your data.
          </p>
        ) : null}
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <p className="mt-1 text-sm text-text-secondary">Send the key as <code>Authorization: Bearer lk_api_...</code>. Everything returns JSON.</p>
            ...
            {API_ENDPOINTS.map((endpoint) => (
              <tr key={endpoint.path} className="border-t border-slate-100 align-top">
                ...
                <td className="py-2 text-text-secondary">
                  {endpoint.what}
                  {endpoint.params === 'none' ? null : <span className="mt-1 block text-xs">{endpoint.params}</span>}
                </td>
              </tr>
            ))}
          </tbody>
